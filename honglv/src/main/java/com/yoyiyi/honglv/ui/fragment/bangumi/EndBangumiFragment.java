 ...
 */
public class EndBangumiFragment extends BaseFragment implements BaseQuickAdapter.RequestLoadMoreListener, SwipeRefreshLayout.OnRefreshListener,
    ...
    public static EndBangumiFragment newInstance(int index) {
        Bundle bundle = new Bundle();
        bundle.putInt(NEWBANGUMI_INDEX, index);
        EndBangumiFragment fragment = new EndBangumiFragment();
    ...
    private void doHttpConnection() {
        HttpManager.getHttpManager().getHttpService()
                .getBangumi(null, null, null, 18, mCurrentPage)
                ...
                .subscribe(animas -> {
                    if (animas.getTotalPage() < mCurrentPage) {
                        ...
                        mTotalCount = animas.getTotalCount();
                        finishTask();
                        mAdapter.addData(animas.getList());
                    }
                }, e -> {
                    if (isLoadingMore) {
                        showErrorView();
